...
状态：P2-0 至 P2-3 的基础查看器交付已完成；旧查看器视觉基线和高级效果适配仍待补，P2-4 已按 2026-09-22 用户确认的上线优先范围完成：生产构建与 Pages 部署成功，线上 7 条路由及关键流程验收通过；完整回归、真机检查保留后续项，详见 [P2-4 报告](p2-4/2026-09-22/README.md)。详见 [P2-0 验证报告](p2-0/2026-09-21/README.md)、[P2-1 交付报告](p2-1/2026-09-21/README.md)、[P2-2 交付报告](p2-2/2026-09-21/README.md) 和 [P2-3 交付报告](p2-3/2026-09-21/README.md)。
...
当前交付入口调整（2026-09-22）：保留最新视觉版本的行为，“查看详情”和“查看示例”进入 `/projects/kaiwu/viewer?debug=1`；个人介绍／KaiwuArt 进入配置页，“3D Viewer”作为能力标签。下表及早期阶段记录保留原规划背景，以此调整为准。
...
- [x] 更新旧链接测试，新增两条路由、三种输入、默认示例、失败恢复和返回导航的行为测试。
- [x] 加入 DOM 链接检查，禁止指向 `kaiwuart.cn` 及其子域的 URL；历史文档不纳入运行时禁用扫描。
- [x] 完成 check、完整 E2E 与静态脚本：13 项单元、132 项浏览器通过（12 项既有跳过）、27 项静态路径通过，覆盖根路径、默认子路径和实际 Pages 前缀。
- [x] 扩展静态校验：两个新页面的 HTML 入口、正确预加载 chunk、刷新、模型及附属资源无 404。
- [x] 自动化检查首页／About／Works／Projects 原有行为与外观；Kaiwu 三引擎、桌面／手机视口、中英和键盘通过，真机另列。
- [ ] 实机检查 iOS Safari 与 Android Chrome 的模型加载、触控和退出重入；未实测项明确留待验收，不以模拟器代替实机结论。
- [x] 更新 README、部署说明与迁移报告，记录验证结果和已知限制；见 [P2-4 报告](p2-4/2026-09-22/README.md)。
- [x] 发布后验证线上详情跳转、深链刷新和模型加载；`7f8cd84` 的 Pages 运行 `35654069835` 成功，7 条线上路由验收通过。

交付：Phase 2 验收报告和已部署构建。退出条件（2026-09-22 用户调整）：生产构建成功，线上关键路由、详情跳转、刷新与默认模型通过；完整 E2E、审计及真机检查不作为发布门禁，未执行或失败项仍如实记录。

## 5. 完成标准

1. 网站可点击入口不再访问旧域名；KaiwuArt 品牌与项目介绍仍保留。
2. 按当前交付入口，“查看详情”打开调试查看器，KaiwuArt 打开迁入的模型配置页；3D Viewer 保留为能力标签。
