...
状态：P2-0 至 P2-3 的基础查看器交付已完成；旧查看器视觉基线和高级效果适配仍待补，P2-4 尚未完成。详见 [P2-0 验证报告](p2-0/2026-09-21/README.md)、[P2-1 交付报告](p2-1/2026-09-21/README.md)、[P2-2 交付报告](p2-2/2026-09-21/README.md) 和 [P2-3 交付报告](p2-3/2026-09-21/README.md)。
...
- [x] 修改 profile、portfolio、ProfileIdentity、KaiwuSection，按链接表切换所有旧域名入口。
- [x] 将站内目标与外链在数据模型中明确区分，更新对应的内容校验。
- [x] 将迁入页面标题、字段、按钮、加载状态和错误提示接入现有中英文语言体系。
- [x] 保留三种输入方式，补充“查看示例”；移动端输入不溢出，按钮支持键盘与清晰焦点。
- [x] 校验非法 JSON、空输入、错误参数和资源加载失败；30 秒超时结束 loading，允许重试。
- [x] 保留 `type=1/2` 的 URL 输入语义；本地 JSON 使用会话存储传递；刷新恢复，缺失或禁用存储时显示明确提示。
- [x] 对用户自定义远程资源的 CORS／网络错误提供说明和重试，本地默认示例不依赖这些服务。

交付：完整的“Projects → 查看详情 → 输入／示例 → 查看器 → 返回”流程。退出条件：中英桌面与移动端均可完成，运行页面中没有旧域名链接。

### P2-3：3D 生命周期与性能

- [x] 将源代码模块级共享状态和可变 PRESET 改为实例状态及深拷贝配置，防止多次访问串扰。
- [x] 清理 RAF、观察器、controls 监听、renderer、材质、纹理及 PMREM；处理加载中离开竞态。当前未迁入 GUI／高级 passes，无相关常驻资源。
- [x] 使用实例内 TextureLoader，保持 `window.createImageBitmap` 不变；桌面与移动 Chromium 验证通过，实机兼容性留待 P2-4。
- [x] 静止时按需渲染、离开停止、后台取消 RAF；WebGL 不可用及上下文丢失可读回退并支持重试。
- [x] 只在进入查看器时加载重型代码和模型；采样确认 Kaiwu 输入页与 homepage 不请求查看器模型／HDR。
- [x] 完成历史归档与当前版本的三轮资源／耗时对比。记录体积增长与剩余约 19 MB 示例开销；高级效果未验证前不启用，不宣称旧效果等价。
...
每一步（P2-0 至 P2-4，以及后续单独交付的子步骤）完成交付后，都必须执行一次 Git commit：先完成该步骤的必要验证，保存可核查结果并更新 Roadmap／交付报告，再提交本步骤相关代码、资源、测试和文档，不累积到下一步才提交。提交信息的标题和正文均使用英文，并在交付回复中记录 commit hash。提交前检查变更范围，不混入无关改动；Git commit 不包含自动 push。

推荐按依赖顺序拆分提交，避免一次提交同时包含依赖调整、迁移、链接替换和所有验收变更。工期在 P2-0 兼容性验证后估算；当前不将未验证的技术风险折算成承诺日期。
